...
function writeReports(artifactsDir, report) {
  ...
  const jsonPath = path.join(artifactsDir, "footprint-report.json");
  const mdPath = path.join(artifactsDir, "footprint-report.md");
...
function toMarkdown(report) {
  const lines = [];
  lines.push("# Footprint E2E Report");
  lines.push("");
